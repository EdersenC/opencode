...
| <nobr><code>{"--auto"}</code></nobr>        |       | Auto-approve safe project-local bash permissions                        |
| <nobr><code>{"--permission-mode"}</code></nobr> |       | Permission mode: `ask` or `auto`                                        |
...
The interactive footer shows live timing as `total`, `work`, and `between`: total session time, active agent working time, and time between user interactions. Use `/permissions` or command palette **Permissions** to switch the current session between **Approve** and **Auto**.
...
| <nobr><code>{"--permissions"}</code></nobr> |       | Comma-separated list of permissions to allow (default: all). Available: `bash`, `read`, `edit`, `glob`, `grep`, `webfetch`, `group`, `task`, `todowrite`, `websearch`, `lsp`, `skill`. Anything omitted is denied. Alias: `--tools` |
