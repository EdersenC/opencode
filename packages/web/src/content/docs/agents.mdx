...
OpenCode comes with three built-in primary agents, **Build**, **Plan**, and **Orchestrate**. We'll
look at these below.
...
OpenCode comes with five built-in subagents, **General**, **Explore**, **Planner**, **Coder**, and **Scout**. We'll look at this below.

---

## Built-in

OpenCode comes with three built-in primary agents and five built-in subagents.
...
An agent for large, ambiguous, or multi-part goals where parallel planning, implementation, review, and reconciliation are useful. It decomposes work into grouped subagent tasks, asks clarifying questions early when requirements are ambiguous, compares multiple plans, coordinates implementation, reviews coder output, redispatches targeted fixes when needed, and synthesizes results.

Orchestrate is designed to batch ready work. After contracts or handoff files exist, it should dispatch every safe, non-overlapping coder slice in the same grouped task batch instead of making you wait through avoidable one-agent-at-a-time phases.

Before it announces an implementation phase, Orchestrate should decide the ready batch first. A "parallel" phase should normally mean one `group` call with multiple coder tasks already in `calls`, using compact `handoff_files` instead of long duplicated prompts.

In normal CLI/TUI, app, and desktop usage, Orchestrate can use the `question` tool when its permissions allow it. Other runtime clients may hide `question` unless `OPENCODE_ENABLE_QUESTION_TOOL` is enabled.
...
A read-only planning agent that creates one concrete implementation plan for a complex task. Use multiple planner agents in parallel through Orchestrate and the Task/Group tools to compare approaches such as minimal viable, robust architecture, risk-first, or integration-first plans.
...
An implementation agent for scoped, high-quality coding work. Users usually do not select Coder directly; Orchestrate dispatches coder agents through grouped Task calls after planning and interface or contract preparation, points them at compact `handoff_files`, then reviews and reconciles their output.
...
`read`, `edit`, `glob`, `grep`, `list`, `bash`, `group`, `task`, `external_directory`, `lsp`, and `skill` accept either a shorthand action (`"allow" | "ask" | "deny"`) or an object of glob/pattern → action for fine-grained control. The remaining keys accept the shorthand action only.
