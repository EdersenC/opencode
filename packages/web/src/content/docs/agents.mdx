...
An agent for large, ambiguous, or multi-part goals where parallel planning, implementation, and review are useful. It decomposes work into grouped subagent tasks, asks clarifying questions early when requirements are ambiguous, compares multiple plans, coordinates implementation, and synthesizes results.
...
An implementation agent for scoped, high-quality coding work. Users usually do not select Coder directly; Orchestrate dispatches coder agents through grouped Task calls after planning and interface or contract preparation.
