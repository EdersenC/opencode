...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"`, `"plan"`, or `"orchestrate"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenCode will fall back to `"build"` with a warning.
...
To auto-approve normal project-local development commands that would otherwise ask, set `permission_mode`:
...
AUTO is intended for trusted local repositories where agents need to run tests, builds, linters, formatters, and other workspace-scoped commands frequently. It does not mean unrestricted shell access: explicit deny rules still win, and commands that escape the project directory, execute downloaded packages, broadly delete the workspace, publish/deploy externally, push to remotes, or perform privileged system mutation still require approval or are denied.
