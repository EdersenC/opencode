...
AUTO is intended for trusted local repositories where agents need to run tests, builds, linters, formatters, and other workspace-scoped commands frequently. It does not mean unrestricted shell access: explicit deny rules still win, and commands that escape the project directory, publish/deploy externally, push to remotes, or perform privileged system mutation still require approval or are denied.
