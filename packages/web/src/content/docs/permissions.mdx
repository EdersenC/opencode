...
Start OpenCode with `--auto` to automatically approve safe project-local bash permission requests that are not explicitly denied.

```bash
opencode --auto
```

You can also use `--permission-mode auto`, including with [`opencode run`](/docs/cli#run).

```bash
opencode run --permission-mode auto "Refactor this module"
```

Explicit `"deny"` rules are still enforced. Auto mode only changes safe project-local bash requests that would otherwise ask for approval. Requests that touch paths outside the project directory, execute downloaded packages, broadly delete the workspace, run deployment/cloud/container/system commands, push/publish, or pipe remote downloads into a shell still ask. Clearly destructive commands such as deleting root or home directories are denied.

Auto mode follows the active session, including subagents launched through `task` and `group` by agents such as `orchestrate`. Agent-level deny rules still win, so read-only agents such as `plan` and planner-style subagents do not gain mutation access just because auto mode is active.

You can make auto mode persistent in config:
...
In direct interactive `opencode run`, type `/permissions` or open the command palette and select **Permissions** to switch between **Approve** and **Auto**. The footer statusline shows `AUTO` while auto mode is active. In the full TUI, use `/permissions` from slash autocomplete or the **Permissions** command to toggle the current session between Approve and Auto.
...
- `group` — launching grouped subagent task calls (matches the group name)
