...
In the TUI, open the command palette and select **Enable auto-approve permissions** or **Disable auto-approve permissions** to change modes. In direct interactive `opencode run`, the footer statusline shows `AUTO` while auto mode is active.
