...
Run multiple nested `task` subagent calls as one logical group and return one combined result after all nested calls finish.
...
Use this when several subagents should work on parts of the same goal and the model should receive one grouped result. The `group` permission controls whether the grouping tool can run; each nested task still uses the existing `task` permission for the selected subagent.

For implementation orchestration, `group` should be used as a real batch: put every ready, non-overlapping coder task in the same `calls` array so they start together. If contracts or handoff files let engine, CLI, tests, docs, adapters, or UI work proceed at the same time, they should be launched together instead of one after another.
